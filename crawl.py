...
async def bfs_crawl(
    ...
    Optionally include sitemap URLs in the initial queue.
    """
    visited: Set[str] = set()
    queue = deque()
    results = []
    ...
    while queue:
        ...
        try:
            ...
            # If depth < max_depth, discover new links
            if depth < max_depth:
...
async def discover_links(url: str, session: aiohttp.ClientSession, user_agent: str) -> List[str]:
    """
    Minimal fetch to get HTML and extract links. We do not follow redirects here.
...
def main():
    ...
    include_sitemap_in_crawl = st.checkbox("Include Sitemap URLs in Crawl")
    sitemap_urls = []
